...
import type {
  DashboardRadialInteraction,
  DashboardWidget,
  NewNoteFilenameFormat,
  PulseCard,
  PulseCardDisplayMode,
  QuickAction,
  QuickActionIconType,
  RadialMode
...
export class ReadyBoardSettingsModal extends Modal {
  ...
  private renderDashboard(s: PluginSettings, save: () => void): void {
    ...
    new Setting(el).setName('Overdrag to new note').setDesc('Pull down past the top of the dashboard to instantly create a new note.')
      .addToggle((t) =>
        t.setValue(s.enableOverdrag).onChange((v) => {
          s.enableOverdrag = v;
          save();
        })
      );
    ...
    new Setting(el).setName('Sidebar side')
      .addDropdown((d) =>
        d.addOption('right', 'Right').addOption('left', 'Left')
          .setValue(s.dashboardSidebarSide ?? 'right')
          .onChange((v) => {
            s.dashboardSidebarSide = v as 'left' | 'right';
            save();
          })
      );
    ...
    new Setting(el)
      ...
      .addToggle((t) =>
        t.setValue(s.dashboardSeparateSettings ?? false).onChange((v) => {
  ...
  private renderGeneral(s: PluginSettings, save: () => void): void {
    ...
    new Setting(el)
      ...
      .addText((t) => {
        t.setPlaceholder('Home.md').setValue(s.homePath).onChange((v) => {
          s.homePath = v.trim();
        });
        this.saveTextOnCommit(t.inputEl, save);
    ...
    new Setting(el).setName('Handedness').setDesc('Right-handed mode right-aligns controls.')
      .addDropdown((d) =>
        d.addOption('left', 'Left-handed').addOption('right', 'Right-handed')
          .setValue(s.handedness ?? 'left').onChange((v) => {
            s.handedness = v as 'left' | 'right';
            save();
          })
      );
    ...
    new Setting(el).setName('Folder')
      .addText((t) => {
        t.setPlaceholder('Inbox').setValue(s.newNoteFolder).onChange((v) => {
    ...
    new Setting(el).setName('Template')
      .addText((t) => {
        t.setPlaceholder('Templates/New Note.md').setValue(s.newNoteTemplate).onChange((v) => {
          s.newNoteTemplate = v.trim();
        });
        this.saveTextOnCommit(t.inputEl, save);
        new FileSuggest(this.app, t.inputEl);
      });
    ...
    const cs = new Setting(el).setName('Custom format').setDesc('Tokens: YYYY YY MM DD HH mm ss')
      .addText((t) => {
        t.setPlaceholder('YYMMDD_HHmmss').setValue(s.newNoteFilenameCustom ?? '')
          .onChange((v) => {
            s.newNoteFilenameCustom = v.trim();
          });
        this.saveTextOnCommit(t.inputEl, save);
      });
  ...
  private renderPulse(s: PluginSettings, save: () => void): void {
    ...
    if (cards.some((c) => c.type === 'inbox')) {
      ...
      new Setting(el).setName('Inbox folder path').setDesc('e.g. "Inbox" or "Capture/Unsorted"')
        .addText((t) => {
          t.setPlaceholder('Inbox').setValue(s.inboxPath ?? '').onChange((v) => {
            s.inboxPath = v.trim();
          });
          this.saveTextOnCommit(t.inputEl, save);
          new FolderSuggest(this.app, t.inputEl);
    ...
    if (cards.some((c) => c.type === 'git')) {
      ...
      new Setting(el).setName('Tap action')
        .addDropdown((dd2) =>
          dd2.addOption('sync', 'Commit and sync').addOption('menu', 'Open git menu')
            .setValue(s.gitPulseCardAction ?? 'sync')
            .onChange((v) => {
              s.gitPulseCardAction = v as 'menu' | 'sync';
              save();
            })
        );
    }
  ...
  private renderRadial(s: PluginSettings, save: () => void): void {
    ...
    new Setting(el).setName('Default mode')
      .addDropdown((d) =>
        d.addOption('breadcrumbs', 'Breadcrumbs').addOption('commands', 'Commands').addOption('recents', 'Recents')
          .setValue(s.radialDefaultMode ?? 'breadcrumbs').onChange((v) => {
            s.radialDefaultMode = v as RadialMode;
            save();
          })
      );
    ...
    new Setting(el).setName('Remember last mode').setDesc('Reopen in whatever mode you last used.')
      .addToggle((t) =>
        t.setValue(s.radialRememberLast ?? false).onChange((v) => {
          s.radialRememberLast = v;
          save();
        })
      );
    ...
    new Setting(el).setName('Connect radial & dashboard').setDesc('Swipe down on the radial to open the dashboard.')
      .addToggle((t) =>
        t.setValue(s.connectSurfaces ?? true).onChange((v) => {
          s.connectSurfaces = v;
          save();
        })
      );
    ...
    new Setting(el).addButton((btn) =>
      btn.setButtonText('Reset slots to defaults').onClick(() => {
  ...
  private renderRadialPreview(el: HTMLElement, commands: QuickAction[]): void {
    ...
    const anchors = SLOTS.map((pos) => `<circle cx="${pos.left}" cy="${pos.top}" r="4" fill="#5a536a" opacity="0.42"/>`).join('');
