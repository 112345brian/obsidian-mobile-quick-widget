...
function guideSvg(mode: RadialMode, slots: readonly (Slot | undefined)[]): string {
  const activeSlots = slots
    .map((slot, index) => ({ pos: SLOTS[index], slot }))
    .filter((item): item is { pos: SlotPos; slot: Slot } => Boolean(item.slot?.onTap && item.pos));

  const spokes = activeSlots.map(({ pos, slot }) => {
  ...
  const anchors = activeSlots.map(({ pos, slot }) => {
    const color = slotColor(slot.kind, mode);
...
function render(root: HTMLElement, ctx: DashboardWidgetContext): void {
    ...
    centerTapTimer = window.setTimeout(() => {
      ...
      if (ctx.settings.radialRememberLast) {
        void ctx.editSettings((s) => {
          if (ctx.surface === 'sidebar' && s.dashboardSeparateSettings) { s.sidebarDashboardRadialLastMode = mode; } else { s.dashboardRadialLastMode = mode; }
        });
      }
      renderCard();
    }, DOUBLE_TAP_MS);
  ...
  const renderCard = (): void => {
    ...
    if (!expanded) {
      ...
      if (interaction === 'tap-toggle') { compact.addEventListener('click', () => { setExpanded(true); }); } else {
    ...
    center.addEventListener('click', (event) => {
      event.stopPropagation();
      if (interaction === 'tap-toggle') { onCenterTapToggle(); } else { setExpanded(false); }
    });
